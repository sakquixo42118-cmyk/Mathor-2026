...
def get_paths() -> tuple[Path, Path, Path]:
    ...
    return input_csv, out_subdir, fig_subdir
...
def main() -> None:
    input_csv, out_subdir, fig_subdir = get_paths()
    ...
    ph_coef = pd.DataFrame({"指标": FEATURES, "ElasticNet系数": elastic_net.named_steps["model"].coef_})
    ...
    risk_coef = pd.DataFrame({"指标": FEATURES, "Logit系数": coef, "OR=exp(coef)": np.exp(coef)})
    ...
    (out_subdir / "q1_multivariable_summary.txt").write_text("\n".join(summary_lines), encoding="utf-8")
