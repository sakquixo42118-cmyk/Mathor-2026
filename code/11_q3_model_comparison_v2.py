...
def _apply_offsets(points_df: pd.DataFrame, x_col: str, y_col: str,
                   x_scale: float = 18.0, y_scale: float = 0.35,
                   round_x: int = 0, round_y: int = 1) -> pd.DataFrame:
    """
    对重合或近重合散点做确定性错位。
    """
    df = points_df.copy()
    ...
    df["_x_bin"] = df[x_col].round(round_x)
    df["_y_bin"] = df[y_col].round(round_y)
    df["_plot_x"] = df[x_col].astype(float)
    df["_plot_y"] = df[y_col].astype(float)
    ...
    for (_, _), idx in df.groupby(["_x_bin", "_y_bin"]).groups.items():
        idx = list(idx)
        if len(idx) <= 1:
            continue
        for j, row_idx in enumerate(idx):
            ox, oy = offset_patterns[j % len(offset_patterns)]
            df.loc[row_idx, "_plot_x"] = df.loc[row_idx, x_col] + ox * x_scale
            df.loc[row_idx, "_plot_y"] = df.loc[row_idx, y_col] + oy * y_scale

    return df.drop(columns=["_x_bin", "_y_bin"])
...
def plot_group_heatmaps(summary_df: pd.DataFrame, plan_type_for_aux: str = "均衡方案"):
    ...
    for value_col, fname, title, cbar_label in [
        ("期末痰湿积分", "q3_compare_group_final_score_heatmap.png",
         f"三套模型比较：平均期末痰湿积分（辅助验证取{plan_type_for_aux}）", "期末痰湿积分"),
        ("6个月总成本", "q3_compare_group_total_cost_heatmap.png",
         f"三套模型比较：平均6个月总成本（辅助验证取{plan_type_for_aux}）", "6个月总成本"),
    ]:
        panels = []
        ...
        all_vals = np.concatenate([p.values.flatten() for p in panels])
        all_vals = all_vals[~np.isnan(all_vals)]
        vmin = float(all_vals.min()) if len(all_vals) else None
        vmax = float(all_vals.max()) if len(all_vals) else None

        fig = plt.figure(figsize=(16, 5.4), dpi=220, constrained_layout=True)
        gs = fig.add_gridspec(1, 4, width_ratios=[1, 1, 1, 0.06])
        axes = [fig.add_subplot(gs[0, i]) for i in range(3)]
        cax = fig.add_subplot(gs[0, 3])

        last_im = None
        for ax, model_name, grp in zip(axes, model_order, panels):
            last_im = ax.imshow(grp.values, aspect="auto", vmin=vmin, vmax=vmax)
        ...
        if last_im is not None:
            ...
            cbar.ax.set_ylabel(cbar_label, fontproperties=ZH_FONT, rotation=90, labelpad=12)
...
def plot_sample_scatter(sample_df: pd.DataFrame, sample_id: int):
    ...
    sub = _apply_offsets(sub, "6个月总成本", "期末痰湿积分", x_scale=20.0, y_scale=0.45, round_x=0, round_y=1)

    fig, ax = plt.subplots(figsize=(8.5, 6.5), dpi=220)
...
def plot_sample_pareto_overlay(sample_id: int, summary_df: pd.DataFrame, pareto_dict: Dict[str, pd.DataFrame]):
    ...
    model_points = summary_df[summary_df["样本ID"] == sample_id].copy()
    model_points = _apply_offsets(model_points, "6个月总成本", "期末痰湿积分", x_scale=16.0, y_scale=0.35, round_x=0, round_y=1)

    fig, ax = plt.subplots(figsize=(8.5, 6.5), dpi=220)
    ax.scatter(
        front[cost_col], front[score_col],
        s=55, alpha=0.75, label="辅助验证帕累托前沿",
        c="#5DA5DA", edgecolors="#2F6EA3", linewidths=0.6, zorder=1
    )
    ...
    for _, row in model_points.iterrows():
        label = f'{row["模型"]}｜{row["方案类型"]}'
        ax.scatter(
            row["_plot_x"], row["_plot_y"],
            s=240 if row["方案类型"] == "疗效最优" else 210,
            marker=marker_map.get(row["方案类型"], "o"),
            label=label,
            c=color_map.get(row["模型"], "#333333"),
            edgecolors="black",
            linewidths=0.9,
            zorder=4
        )
    ...
    ax.legend(prop=ZH_FONT, fontsize=9, loc="best")
    ax.grid(alpha=0.3)
    plt.tight_layout()
